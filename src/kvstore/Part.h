...
class Part : public raftex::RaftPart {
 ...
 private:
  ...
  /**
   * @brief Apply the logs in iterator to state machine, and
   *        return the commit log id and commit log term if succeed
   *
   ...
   *   When the raftLock_ has been acquired before commitLogs is invoked,
   *    needLock is false (e.g. commitLogs by follower).
   ...
   */
  std::tuple<nebula::cpp2::ErrorCode, LogID, TermID> commitLogs(
      std::unique_ptr<LogIterator> iter, bool wait, bool needLock) override;
