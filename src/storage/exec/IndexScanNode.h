...
/**
 * @brief `IndexScanNode` is the base class of the node which need to access disk.
 *        It has two derive class `IndexVertexScanNode` and `IndexEdgeScanNode`.
 *
 ...
 *
 * `Path` is the most important part of `IndexScanNode`.
 * By analyzing `ColumnHint`, it obtains the mode(`Prefix` or `Range`) and
 * range(key of `Prefix` or [start,end) of `Range`) of keys that `IndexScanNode`
 * need to query in kvstore.
 *
 * `Path` not only generate the key to access, but also `qualified` whether the
 * key complies with the columnhint constraint or not.
 *   For example, if there is a truncated string index, we cannot simply compare
 *   bytes to determine whether the current key complies with the columnhints
 *   constraint, the result of `qualified(bytes)` should be `UNCERTAIN` and
 *   `IndexScanNode` will access base data then `Path` reconfirm `ColumnHint`
 *   constraint by `qualified(RowData)`.
 *   In addition to the above examples, there are other cases to deal with.
 * `Path` and it's derive class will dynamic different strategy by `ColumnHint`,
 * `IndexItem`,and `Schema`.
 * All strategy will be added to `strategySet_`(QualifiedStrategySet) during
 * `buildKey`, and executed during `qualified`.
 *
 * `Path` will be reset when `IndexScanNode` execute on a new part.
 *
 * It should be noted that the range generated by `rangepath` is a certain left
 * included and right excluded interval,like [startKey_, endKey_), although
 * `ColumnHint` may have many different constraint ranges
 * (e.g., (x, y],(INF,y),(x,INF)).
 * Because the length of index key is fixed, the way to obtain 
 * **the smallest key greater than 'x'** is to append several '\xFF' after until
 * the length of 'x' is greater than the length of the indexkey.
 *
